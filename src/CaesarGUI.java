...
public class CaesarGUI extends JFrame implements ActionListener {
        ...
        private static String alphabet = "ov8,y:>6;p4<_a+-dwsqbr=19c5m 7nz32e#@!f%$^?u)h&.x(kt*jgli~`";
        ...
        public void encryptText() throws InterruptedException {
                ...
                if(!textNum.equals("")){
                        shift = Integer.parseInt(textNum)%59;
                }
                else{
                        shift = 0;
                }
                ...
                for(int i=0; i<alphabet.length(); i++){
                        alphaMap.put(alphabet.charAt(i), alphabet.charAt((i+shift)%59));
                }
        ...
        public void decryptText() throws InterruptedException{
                ...
                for(int i=0; i<alphabet.length(); i++){
                        alphaMap.put(alphabet.charAt((i+shift)%59), alphabet.charAt(i));
                }
                String inputText = inputTA.getText().toLowerCase();
                String outputText = "";
                for(int j=0; j<inputText.length(); j++){
